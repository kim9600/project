...
<div id="container" style="height: 850px;">
  ...
  <form role="form" action="/review/modify" method="post" enctype="multipart/form-data">
			        ...
			        <table width="800px;" border="0" cellpadding="0" cellspacing="0" class="table_l" style="margin-left: 20px;">
		              ...
		              <tbody>
		                 <tr>
		                    <th>글 번호</th>
		                        <td>
		                           <input type="text"  name="bno" value= '<c:out value="${review.bno }"/>' readonly style="height: 30px; width: 70px;"></td>
		                        </td>
		                 </tr>
			             <tr>
				            <th>제목</th>
				               <td><input class="form-control" size="60" name="title" value='<c:out value="${review.title }"/>'  style="height: 30px;"></td>
			             </tr>
			            ...
			            <tr>
				           <td colspan="2" style="width: 730px; height: 350px;">
					        <textarea style="width: 800px; height: 350px;  resize:none; font-size: 14px; text-align: left;" name="content" ><c:out value="${review.content }" /></textarea>
						 </td>
			          </tr>
			          
			          <tr>
  				           <th>등록일자</th>
				              <td><input type="text" name="regdate" value='<fmt:formatDate value="${review.regdate}" pattern="yyyy/MM/dd"/>' style="height: 30px;" readonly></td>
			           </tr>
			          
			          <tr>
  				           <th>수정일자</th>
				              <td><input type="text" name="updateDate" value='<fmt:formatDate value="${review.updateDate}" pattern="yyyy/MM/dd"/>' style="height: 30px;" readonly></td>
			           </tr>
			           <tr>
			           <th>파일 첨부</th>
			          <td>
			          <div class="thumb">
					<input type="file" name="uploadFile" id="myFile" size="50" value=' <c:out value="${review.images}"/>'>
           					<img src='<c:url value="/img/${review.images}"/>' id="myImg" style="width:30%" alt="Image"/>
					</div>
					</td>
					</tr>
         ...
         <br>
           <!-- 수정,삭제,리스트가기 버튼  -->
            <button type="submit" data-oper='modify'  class="btn btn-primary" style="float: right; width: 66px; margin-left: 5px; ">수정</button>
            <button type="submit" data-oper='remove' class="btn btn-danger" style="float: right; margin-left: 5px;">삭제</button>
            <button type='submit'  data-oper='list' class="btn btn-default" style="float: right;">글목록</button>
   </form>
</div>
